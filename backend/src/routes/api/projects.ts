...
import {
	createDeckPdfFile as defaultCreateDeckPdfFile,
	PdfExportError,
} from "../../collab/pdf-export.ts";

type ProjectsRouterDeps = {
	createDeckPdfFile: typeof defaultCreateDeckPdfFile;
};
...
export function createProjectsRouter(
	deps: ProjectsRouterDeps = { createDeckPdfFile: defaultCreateDeckPdfFile },
) {
	...
	app.get("/:projectId/export.pdf", async (c) => {
		const user = c.get("user");
		if (!user) {
			return c.json({ error: "Unauthorized" }, 401);
		}
		...
		if (!fileId) {
			return c.json({ error: "Missing selected deck file" }, 400);
		}

		let pdfFile: Awaited<ReturnType<typeof defaultCreateDeckPdfFile>>;
		try {
			pdfFile = await deps.createDeckPdfFile(projectId, fileId);
		} catch (error) {
		...
		c.header(
			"Content-Disposition",
			`attachment; filename*=UTF-8''${encodeURIComponent(pdfFile.filename)}`,
		);
		c.header("Cache-Control", "no-store");

		return stream(c, async (s) => {
			const readStream = createReadStream(pdfFile.path);
			s.onAbort(() => {
				readStream.destroy();
				void pdfFile.cleanup();
			});
			try {
